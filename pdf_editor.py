#!/usr/bin/env python3
"""
PDF Editor Local v3 - Usa PyMuPDF para renderizar sin poppler
"""
...
class PDFEditorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        btn_f = tk.Frame(topbar, bg=BG_PANEL)
        btn_f.pack(side="right", padx=12)
        self._topbtn(btn_f, "📂 Abrir PDF",    self.open_pdf,   ACCENT)
        self._topbtn(btn_f, "➕ Insertar PDF",  self.insert_pdf, "#2b6cb0")
        self._topbtn(btn_f, "💾 Guardar",        self.save_pdf,   GREEN)
        ...
        # Izquierda
        left = tk.Frame(main, bg=BG_CARD, width=215)
        left.pack(side="left", fill="y", padx=(10,0), pady=10)
        left.pack_propagate(False)
        self._build_left(left)

        # Derecha — preview
        right = tk.Frame(main, bg=PREVIEW_BG, width=350)
    ...
    def _build_left(self, p):
        def section(title):
            tk.Label(p, text=title, font=("Segoe UI", 8, "bold"),
                     bg=BG_CARD, fg=TEXT_GRAY).pack(anchor="w", padx=14, pady=(14,4))
        ...
        for lbl, cmd, color in [
            ...
            tk.Button(p, text=lbl, command=cmd, bg=color, fg=TEXT_WHITE,
        ...
        for lbl, cmd in [
            ...
            tk.Button(p, text=lbl, command=cmd, bg=BG_CARD, fg=TEXT_WHITE,
                      relief="flat", font=("Segoe UI", 9), cursor="hand2",
    ...
    def _build_preview(self, p):
        hdr = tk.Frame(p, bg=BG_PANEL, height=36)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        tk.Label(hdr, text="👁  Vista Previa", font=("Segoe UI", 9, "bold"),
                 bg=BG_PANEL, fg=TEXT_WHITE).pack(side="left", padx=10, pady=8)

        self.lbl_pv_info = tk.Label(p, text="← Haz clic en una página",
                                    font=("Segoe UI", 8), bg=PREVIEW_BG,
                                    fg=TEXT_GRAY, wraplength=330)
        self.lbl_pv_info.pack(pady=(8,2), padx=8)

        # Canvas de preview con scroll vertical
        cf = tk.Frame(p, bg=PREVIEW_BG)
        cf.pack(fill="both", expand=True, padx=4, pady=4)
        self.pv_canvas = tk.Canvas(cf, bg=PREVIEW_BG, highlightthickness=0)
        ...
        self._pv_placeholder = self.pv_canvas.create_text(
            170, 200,
            text="Haz clic en\nuna miniatura\npara previsualizar",
            font=("Segoe UI", 11), fill=TEXT_GRAY, justify="center"
        )
        self._pv_img_item = None

        # Spinner
        self.lbl_spinner = tk.Label(p, text="", font=("Segoe UI", 8),
                                    bg=PREVIEW_BG, fg=YELLOW)
        self.lbl_spinner.pack(pady=2)

        # Navegación
        nav = tk.Frame(p, bg=BG_CARD, height=40)
        nav.pack(fill="x", side="bottom")
        nav.pack_propagate(False)
        tk.Button(nav, text="◀", command=self._pv_prev, bg=BG_PANEL, fg=TEXT_WHITE,
        ...
        tk.Button(nav, text="▶", command=self._pv_next, bg=BG_PANEL, fg=TEXT_WHITE,
                  relief="flat", font=("Segoe UI", 10), cursor="hand2",
                  padx=12, pady=4, bd=0).pack(side="right", padx=6, pady=5)

    # ── Grid central ─────────────────────────
    def _build_center(self, p):
        tb = tk.Frame(p, bg=BG_CARD, height=36)
        tb.pack(fill="x")
        ...
        tk.Label(tb, text="Zoom:", font=("Segoe UI", 8),
                 bg=BG_CARD, fg=TEXT_GRAY).pack(side="right", padx=(0,4))
        ...
        cf = tk.Frame(p, bg=BG_DARK)
        cf.pack(fill="both", expand=True, pady=(6,0))
        self.canvas = tk.Canvas(cf, bg=BG_DARK, highlightthickness=0)
        ...
        # Progress bar
        self.prog_frame = tk.Frame(p, bg=BG_DARK)
    ...
    def _refresh_grid(self):
        ...
        if not self.pages:
            tk.Label(self.inner, text="📂\n\nAbre un PDF para ver sus páginas",
                     font=("Segoe UI", 14), bg=BG_DARK, fg=TEXT_GRAY,
    ...
    def _make_thumb(self, idx, page, row, col, tw, th):
        ...
        bc   = "#f6c90e" if prev else (THUMB_SEL if sel else BORDER)
        bw   = 3 if prev else (2 if sel else 1)

        frame = tk.Frame(self.inner, bg=THUMB_SEL if sel else THUMB_BG,
        ...
        if img:
            ...
            lbl_img = tk.Label(frame, text="📄", font=("Segoe UI", 26),
                               bg=THUMB_BG, width=tw//10, height=th//20, cursor="hand2")
        lbl_img.pack(padx=4, pady=(4,2))

        lbl_n = tk.Label(frame, text=f"Pág. {idx+1}", font=("Segoe UI", 8, "bold"),
                         bg=THUMB_SEL if sel else THUMB_BG,
                         fg=TEXT_WHITE if sel else TEXT_GRAY)
        lbl_n.pack(pady=(0,4))
        if prev:
            tk.Label(frame, text="👁", font=("Segoe UI", 7),
                     bg=THUMB_BG, fg="#f6c90e").pack(pady=(0,2))
